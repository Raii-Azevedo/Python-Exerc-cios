print ('''EX 015. Escreva um programa que pergunte a quantidade de Km percorridos por um carro alugado e a quantidade 
de dias pelos quais ele foi alugado. Calcule o preço a pagar, sabendo que o carro custa R$ 60 por dia e R$ 0,15 por Km rodado.''')
Km = float (input ('Quantos KM o carro percorreu? '))
D = int (input ('Quantos dias de aluguel? '))
Preço= (D * 60) + (Km * 0.15)
print (f'''Você alugou o carro por {D} dias
Percorreu {Km:.1f}Km com ele
O total a pagar é R${Preço:.2f}''')
